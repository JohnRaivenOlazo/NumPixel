...
const Header = ({ setCalculatorActive }) => { // Receive setCalculatorActive as a prop
  ...
  useEffect(() => {
    ...
    const handleCloseButtonClick = () => {
      ...
      macWindow.classList.remove('minimize');
      setCalculatorActive(false); // Update calculatorActive state to false
    };
    ...
    <>
      <div id="header" className={`header z-50 relative`}>
        <div className="relative top-0 w-full h-screenplus flex flex-col justify-center items-start z-20">
          <video autoPlay loop muted preload="auto" src={headerVideo} type="video/mp4" className="absolute top-0 h-full w-full object-cover z-0"></video>
          <Fade>
